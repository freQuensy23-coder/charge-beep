...
wait_running "$before"
after=$(pid)
# Real AppKit window and controls, with a debug-only native driver; no Accessibility bypass.
debug=$(swift build --show-bin-path)
CHARGE_BEEP_HOME="$scratch/settings" "$debug/ChargeBeepUI" _test-ui "$scratch/ui-result" &
uipid=$!
for _ in {1..30}; do
...
echo 'PASS: installed service, crash recovery, real window close, upgrade, uninstall.'
